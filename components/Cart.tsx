  ...
  return (
    <>
      ...
      <aside className="vp-cart-desktop">
        <div className="vp-cart-row"><div className="vp-cart-order-heading"><h2>{tableName || "Đơn mang đi"}</h2>{onChangeService && <button className="vp-cart-change-service" type="button" onClick={onChangeService}>ĐỔI HÌNH THỨC</button>}</div><button className="vp-button vp-button--secondary" type="button" onClick={onClearAll} disabled={!hasItems}>Làm mới</button></div>
        {hasItems ? (
          <ul className="vp-cart-list">
            {items.map((item) => <CartLine key={item.product.id} item={item} onChange={onUpdateItem} onRemove={onRemoveItem} />)}
          </ul>
