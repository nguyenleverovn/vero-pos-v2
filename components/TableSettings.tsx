...
export function TableSettings({ role }: { role: StoreRole }) {
  ...
  return <section className={styles.card} id="tables">
    ...
    {editable && <div className={styles.builders}>
      ...
      <form onSubmit={addTable}><label><span>Tên bàn</span><input value={tableName} onChange={(event) => setTableName(event.target.value)} placeholder="Bàn 01" /></label>{(config.areas.length > 1 || config.areas.some((area) => area.name !== "Khu vực chung")) && <label><span>Khu vực</span><select value={areaId} onChange={(event) => setAreaId(event.target.value)}>{config.areas.map((area) => <option value={area.id} key={area.id}>{area.name}</option>)}</select></label>}<button>THÊM BÀN</button></form>
    </div>}
