...
export default function VeroPosPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (activeTables.length === 0) {
        const counter = { mode: "counter" } as const;
        saveSaleContext(counter);
        setSaleContext(counter);
  ...
  if (activeTables.length > 0 && !saleContext) {
    return <DiningChooser config={dining} openOrders={openOrders} onCounter={() => { clearCart(); setItems([]); const context = { mode: "counter" } as const; saveSaleContext(context); setSaleContext(context); }} onTable={(tableId) => { const context = { mode: "table", tableId } as const; const tableOrder = openOrders.find((order) => order.tableId === tableId); const nextItems = hydrateTableOrder(tableOrder, catalog.products); saveCart(nextItems); setItems(nextItems); saveSaleContext(context); setSaleContext(context); }} />;
  }
  ...
  function changeService() {
    ...
    clearSaleContext();
    setItems([]);
    setSaleContext(activeTables.length ? null : { mode: "counter" });
  }
